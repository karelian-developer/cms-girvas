...
class PageForms implements InterfacePage
{
  ...
  public function assembly() : void
  {
    ...
    $formsData = array_slice($formsData, $paginationItemCurrent * $paginationItemsOnPage, $paginationItemsOnPage);

    $pagination = new Pagination($this->CMSCore, count($formsData), $paginationItemsOnPage, $paginationItemCurrent);
    $pagination->assembly();

    foreach ($formsData as $dataIndex => $data) {
      ...
      $formsDataItemsAssembled[] = ThemeCollector::assemblyFileContent(
        ...
        [
          ...
          'FORM_DATA_CREATED_DATE_TIMESTAMP' => $createdUnixTimestamp
        ]
      );
    }
