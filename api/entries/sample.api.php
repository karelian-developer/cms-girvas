...
if ($system_core->urlp->get_path(4) == 'categories') {
  $api_file_path = sprintf('%s/api/entries/sample/categories.api.php', CMS_ROOT_DIRECTORY);
  include_once($api_file_path);
} else if ($system_core->urlp->get_path(3) == 'sorttypes') {
  $api_file_path = sprintf('%s/api/entries/sample/sorttypes.api.php', CMS_ROOT_DIRECTORY);
  include_once($api_file_path);
} else {
  define('API_HANDLERS_ABSOLUTE_PATH', sprintf('%s/api/entries/sample', CMS_ROOT_DIRECTORY));
