...
namespace core\PHPLibrary\Template {
  ...
  final class Collector {
     ...
     * @return string
     */
    public static function assembly_styles(Template $theme, array $stylesArray) : string {
      ...
      foreach ($stylesArray as $style) {
        if (array_key_exists('href', $style) && array_key_exists('rel', $style)) {
          ...
          if (!$styleIsCore) {
            $styleHref = ($theme->get_category() != 'default') ? '/templates/' . $theme->get_category() . '/' . $theme->get_name() . '/' .$style['href'] : '/templates/' . $theme->get_name() . '/' . $style['href'];
          }
     ...
     * @return string
     */
    public static function assembly_locale(string $themeString, SystemCoreLocale|TemplateLocale|ModuleLocale $locale) : string {
      $themeTransformed = $themeString;

      $localeData = $locale->get_data();
      if (!empty($localeData)) {
     ...
     * @return string
     */
    public static function assembly_locale_markdown(string $themeString, SystemCoreLocale|TemplateLocale|ModuleLocale $locale) : string {
      $themeTransformed = $themeString;
      $localeRegistryArray = $locale->get_registry_array();
      $localeCorePath = $locale->get_data_path();
    ...
    public static function assembly_logic(SystemCore $CMSCore, string $themeString) : string {
      $themeTransformed = $themeString;
      ...
      if (preg_match(self::TEMPLATE_LOGIC_IF_PATTERN, $themeTransformed, $matches)) {
        ...
        if ($defineFunctionReturned) {
          $themeTransformed = str_replace($matches[0], self::assembly_logic($matches[4]), $themeTransformed);
        } else {
          $themeTransformed = '';
        }
      }
     ...
     *
     * @param  mixed $filePath Полный путь до файла
     * @param  mixed $themeVariables Массив с тегами шаблона и их значениями
     * @return string
     */
    public static function assembly_file_content(Template $theme, string $filePath, array $themeVariables) : string {
      /** @var string $filePath Полный путь до шаблона */
      $filePath = $theme->get_path() . '/' . $filePath;
